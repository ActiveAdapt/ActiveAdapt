...
def mixtrain(method,training_t_set_name,loop_count, s_batch_size=3,t_batch_size=1,max_epoch=7):
  args = parse_args()
  ...
  s_imdb, s_roidb, s_ratio_list, s_ratio_index = combined_roidb("cleanlemur_2007_train_s")
  s_train_size = len(s_roidb)

  t_imdb, t_roidb, t_ratio_list, t_ratio_index = combined_roidb("cleanlemur_2007_"+training_t_set_name+str(loop_count))
  ...
  s_sampler_batch = sampler(s_train_size, s_batch_size)
  t_sampler_batch=sampler(t_train_size,t_batch_size)

  s_dataset = roibatchLoader(s_roidb, s_ratio_list, s_ratio_index, s_batch_size, \
                           s_imdb.num_classes, training=True)

  s_dataloader = torch.utils.data.DataLoader(s_dataset, batch_size=s_batch_size,
                            sampler=s_sampler_batch, num_workers=args.num_workers)


  t_dataset=roibatchLoader(t_roidb, t_ratio_list, t_ratio_index, t_batch_size, \
                           t_imdb.num_classes, training=True)

  t_dataloader = torch.utils.data.DataLoader(t_dataset, batch_size=t_batch_size,
  ...
  iters_per_epoch = int(s_train_size/s_batch_size)#int(s_train_size / args.s_batch_size)
  #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,args.max_epochs)
  if args.use_tfboard:
    from tensorboardX import SummaryWriter
    logger = SummaryWriter("logs")

  for epoch in range(args.start_epoch, max_epoch + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      if step % args.disp_interval == 0:
        ...
        start = time.time()
    if epoch==max_epoch:
      save_name = os.path.join(output_dir, 'AL_cleanlemur_error_'+method+'_loop'+str(loop_count)+'.pth') 
      save_checkpoint({
