...
def finetune(method,training_t_set_name,loop_count,load_name,save,batch_size=1,max_epoch=2,minibatch_flag=False):
  setup_seed(3)
  args = parse_args()
  ...
  if minibatch_flag==True:
    ...
    for m_t in range(minibatch_time):
      tmp_minibatch_set=open("/root/code/faster-rcnn.pytorch/data/"+args.dataset+"/VOC2007/ImageSets/Main/tmp_minibatch_"+method+str(m_t)+".txt","w")
      tmp_minibatch_list=[]
      ...
      imdb, roidb, ratio_list, ratio_index = combined_roidb(args.dataset+"_2007_tmp_minibatch_"+method+str(m_t))
    ...
    imdb, roidb, ratio_list, ratio_index = combined_roidb(args.dataset+"_2007_"+training_t_set_name+str(loop_count))
    train_size = len(roidb)  # add flipped         image_index*2
    # train set
  ...
  for epoch in range(args.start_epoch, max_epoch + 1):
    ...
    if (epoch==1) and (save==True):#max_epoch:
      save_name = os.path.join(output_dir,"finetune_CL_"+args.dataset+"_"+method+"_loop"+str(loop_count)+".pth")
      save_checkpoint({
