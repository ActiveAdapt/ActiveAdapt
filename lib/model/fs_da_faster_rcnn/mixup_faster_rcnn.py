...
class _fasterRCNN(nn.Module):
    ...
    def baggies_mixup(self, batch_size, im_data, im_info, gt_boxes, num_boxes, tgt_im_data,tgt_gt_boxes,tgt_num_boxes):
        mix_lambda=max(0,min(1,np.random.beta(1.5,1.5)))
        #mix_lambda=random.uniform(0.7,1)
        mix_height = max(im_data.size(2), tgt_im_data.size(2))
        mix_width = max(im_data.size(3), tgt_im_data.size(3))
        ...
        for mix_idx in range(batch_size):
            mix_im_info[mix_idx][0] = mix_height
            mix_im_info[mix_idx][1] = mix_width
            mix_im_info[mix_idx][2] = 1.
            
            mix_gt_boxes[mix_idx][num_boxes[mix_idx]:(num_boxes[mix_idx]+tgt_num_boxes[0])]=tgt_gt_boxes[0].detach().clone()[:tgt_num_boxes[0]]
            
            mix_num_boxes[mix_idx] = mix_num_boxes[mix_idx]+tgt_num_boxes[0].detach().clone()
            
            mix_img[mix_idx,:,:im_data.size(2),:im_data.size(3)] = im_data[mix_idx].detach().clone().cpu().numpy()*mix_lambda
            mix_img[mix_idx,:,:tgt_im_data.size(2),:tgt_im_data.size(3)] += tgt_im_data[0].detach().clone().cpu().numpy()*(1-mix_lambda)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
        ...
        mix_im_data, mix_im_info, mix_gt_boxes, mix_num_boxes = self.baggies_mixup(batch_size, im_data, im_info, gt_boxes, num_boxes, tgt_im_data,tgt_gt_boxes,tgt_num_boxes)
        ...
        # feed base feature map tp RPN to obtain rois
        mix_rois, mix_rpn_loss_cls, mix_rpn_loss_bbox = self.RCNN_rpn(mix_base_feat, mix_im_info, mix_gt_boxes, mix_num_boxes)

        # if it is training phrase, then use ground trubut bboxes for refining
        if self.training:
            mix_roi_data = self.RCNN_proposal_target(mix_rois, mix_gt_boxes, mix_num_boxes)
            mix_rois, mix_rois_label, mix_rois_target, mix_rois_inside_ws, mix_rois_outside_ws = mix_roi_data

            mix_rois_label = Variable(mix_rois_label.view(-1).long())
        ...
        if cfg.POOLING_MODE == 'align':
            mix_pooled_feat = self.RCNN_roi_align(mix_base_feat, mix_rois.view(-1, 5))
        elif cfg.POOLING_MODE == 'pool':
            mix_pooled_feat = self.RCNN_roi_pool(mix_base_feat, mix_rois.view(-1,5))
        ...
        if self.training and not self.class_agnostic:
            ...
            mix_bbox_pred_select = torch.gather(mix_bbox_pred_view, 1, mix_rois_label.view(mix_rois_label.size(0), 1, 1).expand(mix_rois_label.size(0), 1, 4))
