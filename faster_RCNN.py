...
def get_fasterRCNN_resnet(num_classes: int,
    ...
    with torch.no_grad():
        ...
        random_input = torch.rand(size=(1, 3, 512, 512))
        # pretrained_model  = torch.load('/usr/xtmp/hk216/lemurs/model.pkl') 
        # pretrained_model.eval()
        
        features = backbone(random_input)
        print(features.keys()) #torch.Size([1, 1024, 32, 32])

    if isinstance(features, torch.Tensor):
        features = OrderedDict([('0', features)])

    featmap_names = [key for key in features.keys() if key.isnumeric()] #this should be ['0']
...
class FasterRCNN_lightning(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        gt_boxes = [from_dict_to_boundingbox(target, name=name, groundtruth=True, testStep = True) for target, name in zip(y, x_name)]
        gt_boxes = list(chain(*gt_boxes))

        pred_boxes = [from_dict_to_boundingbox(pred, name=name, groundtruth=False, testStep = True) for pred, name in zip(preds, x_name)]
        pred_boxes = list(chain(*pred_boxes))
    ...
    def configure_optimizers(self):
        ...
        # optimizer = torch.optim.Adam(self.model.parameters()) #modified line
